#!/bin/bash
# gloss-play_loves-labours-lost.sh - Analyze the entirety of Love's Labour's Lost
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_loves-labours-lost.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== Love's Labour's Lost - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== Love's Labour's Lost - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
...
echo "Analyzing Love's Labour's Lost..."
echo "Play file: $PLAY"
...
echo "Play: Love's Labour's Lost"
echo "Total scenes: $TOTAL_SCENES"
