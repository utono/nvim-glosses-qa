...
#
# Usage: ./gloss-play_henry-vi-part-2.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
echo ""
echo "--- Act 4, Scene 9 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 4 9 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 4, Scene 9 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 4 9 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 4, Scene 10 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 4 10 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 4, Scene 10 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 4 10 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
