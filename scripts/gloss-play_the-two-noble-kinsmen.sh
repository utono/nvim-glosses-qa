#!/bin/bash
# gloss-play_the-two-noble-kinsmen.sh - Analyze the entirety of The Two Noble Kinsmen
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_the-two-noble-kinsmen.sh [OPTIONS] [FILTER]
#
# Options:
...
PLAY="/home/mlj/utono/literature/shakespeare-william/gutenberg/the_two_noble_kinsmen.txt"
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== The Two Noble Kinsmen - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0

    # Act 1 Prologue
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 0 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Prologue: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Prologue: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 1 Scene 1
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== The Two Noble Kinsmen - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0

    if python "$ANALYZER" "$PLAY" 1 0 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Prologue"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Prologue - NOT FOUND"
    fi
...
echo "Analyzing The Two Noble Kinsmen..."
echo "Play file: $PLAY"
...
# Act 1 Prologue
if matches_filter 1 0; then
    echo "--- Act 1, Prologue ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 0 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Prologue - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 0 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Prologue: exit code $?"
                echo "[FAILED] Act 1, Prologue"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 0 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
echo "Play: The Two Noble Kinsmen"
echo "Total scenes: $TOTAL_SCENES"
...
if [ "$ERRORS" -gt 0 ]; then
    ...
    echo "Output directory: ~/utono/literature/glosses/the-two-noble-kinsmen/"
    exit 0
fi
