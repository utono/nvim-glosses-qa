#!/bin/bash
# gloss-play_othello.sh - Analyze the entirety of Othello
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_othello.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
PLAY="/home/mlj/utono/literature/shakespeare-william/gutenberg/othello_gut.txt"
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== Othello - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
...
if [ "$ERRORS" -gt 0 ]; then
    ...
    echo "Output directory: ~/utono/literature/glosses/othello/"
    exit 0
fi
