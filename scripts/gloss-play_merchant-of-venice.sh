#!/bin/bash
# gloss-play_merchant-of-venice.sh - Analyze the entirety of The Merchant of Venice
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_merchant-of-venice.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== The Merchant of Venice - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== The Merchant of Venice - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
...
echo "Analyzing The Merchant of Venice..."
echo "Play file: $PLAY"
...
echo ""
echo "--- Act 2, Scene 8 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 2 8 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 2, Scene 8 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 2 8 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 2, Scene 9 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 2 9 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 2, Scene 9 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 2 9 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo "Play: The Merchant of Venice"
echo "Total scenes: $TOTAL_SCENES"
