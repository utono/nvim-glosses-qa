...
#
# Usage: ./gloss-play_henry-v.sh [OPTIONS] [FILTER]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    ...
    # Act 2 Prologue
    OUTPUT=$(python "$ANALYZER" "$PLAY" 2 0 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 2 Prologue: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 2 Prologue: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 2 Scene 1
    ...
    # Act 3 Prologue
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 0 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Prologue: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Prologue: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 1
    ...
    # Act 4 Prologue
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 0 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Prologue: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Prologue: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 1
    ...
    # Act 5 Prologue
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 0 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Prologue: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Prologue: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 1
...
if matches_filter 2 0; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 2 0 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 2 0 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 3 0; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 0 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 3 0 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 4 0; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 0 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 0 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 5 0; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 0 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 5 0 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
