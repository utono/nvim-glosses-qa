#!/bin/bash
# gloss-play_macbeth.sh - Analyze the entirety of Macbeth
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_macbeth.sh [OPTIONS] [FILTER]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    ...
    # Act 5 Scene 7
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 7 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 7: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 7: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 8
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 8 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 8: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 8: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== Macbeth - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
    ...
    if python "$ANALYZER" "$PLAY" 5 7 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 7"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 7 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 5 8 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 8"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 8 - NOT FOUND"
    fi
...
# Act 5, Scene 7
if matches_filter 5 7; then
    echo "--- Act 5, Scene 7 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 7 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 7 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 7 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 7: exit code $?"
                echo "[FAILED] Act 5, Scene 7"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 7 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 5, Scene 8
if matches_filter 5 8; then
    echo "--- Act 5, Scene 8 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 8 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 8 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 8: exit code $?"
                echo "[FAILED] Act 5, Scene 8"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
