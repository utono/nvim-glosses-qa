...
#
# Usage: ./gloss-play_macbeth.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
echo ""
echo "--- Act 5, Scene 7 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 5 7 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 5, Scene 7 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 5 7 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 5, Scene 8 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 5 8 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 5, Scene 8 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 5 8 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
