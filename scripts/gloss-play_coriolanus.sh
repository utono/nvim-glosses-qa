#!/bin/bash
# gloss-play_coriolanus.sh - Analyze the entirety of Coriolanus
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_coriolanus.sh [OPTIONS] [FILTER]
#
# Options:
...
PLAY="/home/mlj/utono/literature/shakespeare-william/gutenberg/coriolanus_gut.txt"
...
if [ -n "$STATUS_ONLY" ]; then
    ...
    # Act 1 Scene 6
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 6 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 6: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 6: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 1 Scene 7
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 7 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 7: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 7: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 1 Scene 8
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 8 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 8: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 8: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 1 Scene 9
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 9 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 9: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 9: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 1 Scene 10
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 10 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 10: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 10: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 2 Scene 1
    ...
    # Act 5 Scene 3
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 3 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 3: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 3: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 4
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 4 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 4: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 4: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 5
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 5 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 5: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 5: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 6
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 6 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 6: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 6: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== Coriolanus - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
    ...
    if python "$ANALYZER" "$PLAY" 1 6 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 6"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 6 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 1 7 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 7"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 7 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 1 8 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 8"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 8 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 1 9 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 9"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 9 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 1 10 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 10"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 10 - NOT FOUND"
    fi
    ...
    if python "$ANALYZER" "$PLAY" 5 4 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 4"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 4 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 5 5 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 5"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 5 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 5 6 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 6"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 6 - NOT FOUND"
    fi
...
# Act 1, Scene 6
if matches_filter 1 6; then
    echo "--- Act 1, Scene 6 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 6 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 6 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 6 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 6: exit code $?"
                echo "[FAILED] Act 1, Scene 6"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 6 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 1, Scene 7
if matches_filter 1 7; then
    echo "--- Act 1, Scene 7 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 7 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 7 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 7 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 7: exit code $?"
                echo "[FAILED] Act 1, Scene 7"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 7 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 1, Scene 8
if matches_filter 1 8; then
    echo "--- Act 1, Scene 8 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 8 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 8 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 8: exit code $?"
                echo "[FAILED] Act 1, Scene 8"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 1, Scene 9
if matches_filter 1 9; then
    echo "--- Act 1, Scene 9 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 9 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 9 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 9: exit code $?"
                echo "[FAILED] Act 1, Scene 9"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 1, Scene 10
if matches_filter 1 10; then
    echo "--- Act 1, Scene 10 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 10 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 10 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 10: exit code $?"
                echo "[FAILED] Act 1, Scene 10"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
if matches_filter 5 3; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 3 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 5 3 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
# Act 5, Scene 4
if matches_filter 5 4; then
    echo "--- Act 5, Scene 4 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 4 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 4 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 4 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 4: exit code $?"
                echo "[FAILED] Act 5, Scene 4"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 4 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 5, Scene 5
if matches_filter 5 5; then
    echo "--- Act 5, Scene 5 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 5 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 5 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 5 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 5: exit code $?"
                echo "[FAILED] Act 5, Scene 5"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 5 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 5, Scene 6
if matches_filter 5 6; then
    echo "--- Act 5, Scene 6 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 6 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 6 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 6 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 6: exit code $?"
                echo "[FAILED] Act 5, Scene 6"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 6 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
if [ "$ERRORS" -gt 0 ]; then
    ...
    echo "Output directory: ~/utono/literature/glosses/coriolanus/"
    exit 0
fi
