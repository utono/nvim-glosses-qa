...
#
# Usage: ./gloss-play_antony-and-cleopatra.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
echo ""
echo "--- Act 2, Scene 6 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 2 6 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 2, Scene 6 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 2 6 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 2, Scene 7 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 2 7 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 2, Scene 7 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 2 7 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 3, Scene 9 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 3 9 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 3, Scene 9 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 3 9 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 3, Scene 10 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 3 10 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 3, Scene 10 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 3 10 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
