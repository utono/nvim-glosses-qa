#!/bin/bash
# gloss-play_antony-and-cleopatra.sh - Analyze the entirety of Antony and Cleopatra
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_antony-and-cleopatra.sh [OPTIONS] [FILTER]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== Antony and Cleopatra - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0
    ...
    # Act 1 Scene 4
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 4 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 4: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 4: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 1 Scene 5
    OUTPUT=$(python "$ANALYZER" "$PLAY" 1 5 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 5: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 1 Scene 5: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 2 Scene 1
    ...
    # Act 3 Scene 6
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 6 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 6: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 6: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 7
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 7 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 7: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 7: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 8
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 8 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 8: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 8: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 9
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 9 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 9: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 9: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 10
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 10 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 10: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 10: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 11
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 11 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 11: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 11: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 12
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 12 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 12: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 12: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 3 Scene 13
    OUTPUT=$(python "$ANALYZER" "$PLAY" 3 13 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 13: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 3 Scene 13: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 1
    ...
    # Act 4 Scene 3
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 3 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 3: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 3: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 4
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 4 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 4: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 4: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 5
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 5 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 5: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 5: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 6
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 6 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 6: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 6: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 7
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 7 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 7: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 7: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 8
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 8 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 8: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 8: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 9
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 9 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 9: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 9: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 10
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 10 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 10: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 10: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 11
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 11 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 11: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 11: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 12
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 12 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 12: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 12: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 13
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 13 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 13: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 13: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 14
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 14 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 14: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 14: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 4 Scene 15
    OUTPUT=$(python "$ANALYZER" "$PLAY" 4 15 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 15: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 4 Scene 15: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 1
    ...
    # Act 5 Scene 2
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 2 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 2: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 2: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== Antony and Cleopatra - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
    ...
    if python "$ANALYZER" "$PLAY" 1 4 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 4"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 4 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 1 5 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 1 Scene 5"
    else
        ((INVALID++))
        echo "  ✗ Act 1 Scene 5 - NOT FOUND"
    fi
    ...
    if python "$ANALYZER" "$PLAY" 3 8 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 3 Scene 8"
    else
        ((INVALID++))
        echo "  ✗ Act 3 Scene 8 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 3 9 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 3 Scene 9"
    else
        ((INVALID++))
        echo "  ✗ Act 3 Scene 9 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 3 10 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 3 Scene 10"
    else
        ((INVALID++))
        echo "  ✗ Act 3 Scene 10 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 3 11 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 3 Scene 11"
    else
        ((INVALID++))
        echo "  ✗ Act 3 Scene 11 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 3 12 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 3 Scene 12"
    else
        ((INVALID++))
        echo "  ✗ Act 3 Scene 12 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 3 13 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 3 Scene 13"
    else
        ((INVALID++))
        echo "  ✗ Act 3 Scene 13 - NOT FOUND"
    fi
    ...
    if python "$ANALYZER" "$PLAY" 4 9 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 9"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 9 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 4 10 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 10"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 10 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 4 11 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 11"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 11 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 4 12 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 12"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 12 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 4 13 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 13"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 13 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 4 14 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 14"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 14 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 4 15 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 4 Scene 15"
    else
        ((INVALID++))
        echo "  ✗ Act 4 Scene 15 - NOT FOUND"
    fi
...
# Act 1, Scene 4
if matches_filter 1 4; then
    echo "--- Act 1, Scene 4 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 4 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 4 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 4 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 4: exit code $?"
                echo "[FAILED] Act 1, Scene 4"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 4 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 1, Scene 5
if matches_filter 1 5; then
    echo "--- Act 1, Scene 5 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 1 5 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 1, Scene 5 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 1 5 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 1 Scene 5: exit code $?"
                echo "[FAILED] Act 1, Scene 5"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 1 5 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
if matches_filter 3 6; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 6 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 3 6 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 3 7; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 7 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 3 7 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
# Act 3, Scene 8
if matches_filter 3 8; then
    echo "--- Act 3, Scene 8 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 8 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 3, Scene 8 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 3 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 3 Scene 8: exit code $?"
                echo "[FAILED] Act 3, Scene 8"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 3 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 3, Scene 9
if matches_filter 3 9; then
    echo "--- Act 3, Scene 9 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 9 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 3, Scene 9 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 3 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 3 Scene 9: exit code $?"
                echo "[FAILED] Act 3, Scene 9"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 3 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 3, Scene 10
if matches_filter 3 10; then
    echo "--- Act 3, Scene 10 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 10 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 3, Scene 10 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 3 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 3 Scene 10: exit code $?"
                echo "[FAILED] Act 3, Scene 10"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 3 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 3, Scene 11
if matches_filter 3 11; then
    echo "--- Act 3, Scene 11 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 11 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 3, Scene 11 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 3 11 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 3 Scene 11: exit code $?"
                echo "[FAILED] Act 3, Scene 11"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 3 11 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 3, Scene 12
if matches_filter 3 12; then
    echo "--- Act 3, Scene 12 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 12 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 3, Scene 12 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 3 12 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 3 Scene 12: exit code $?"
                echo "[FAILED] Act 3, Scene 12"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 3 12 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 3, Scene 13
if matches_filter 3 13; then
    echo "--- Act 3, Scene 13 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 3 13 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 3, Scene 13 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 3 13 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 3 Scene 13: exit code $?"
                echo "[FAILED] Act 3, Scene 13"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 3 13 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
if matches_filter 4 3; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 3 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 3 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 4 4; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 4 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 4 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 4 5; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 5 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 5 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 4 6; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 6 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 6 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 4 7; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 7 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 7 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
if matches_filter 4 8; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 8 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 4 8 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
# Act 4, Scene 9
if matches_filter 4 9; then
    echo "--- Act 4, Scene 9 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 9 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 9 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 9: exit code $?"
                echo "[FAILED] Act 4, Scene 9"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 4, Scene 10
if matches_filter 4 10; then
    echo "--- Act 4, Scene 10 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 10 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 10 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 10: exit code $?"
                echo "[FAILED] Act 4, Scene 10"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 4, Scene 11
if matches_filter 4 11; then
    echo "--- Act 4, Scene 11 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 11 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 11 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 11 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 11: exit code $?"
                echo "[FAILED] Act 4, Scene 11"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 11 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 4, Scene 12
if matches_filter 4 12; then
    echo "--- Act 4, Scene 12 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 12 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 12 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 12 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 12: exit code $?"
                echo "[FAILED] Act 4, Scene 12"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 12 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 4, Scene 13
if matches_filter 4 13; then
    echo "--- Act 4, Scene 13 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 13 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 13 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 13 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 13: exit code $?"
                echo "[FAILED] Act 4, Scene 13"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 13 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 4, Scene 14
if matches_filter 4 14; then
    echo "--- Act 4, Scene 14 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 14 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 14 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 14 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 14: exit code $?"
                echo "[FAILED] Act 4, Scene 14"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 14 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 4, Scene 15
if matches_filter 4 15; then
    echo "--- Act 4, Scene 15 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 4 15 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 4, Scene 15 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 4 15 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 4 Scene 15: exit code $?"
                echo "[FAILED] Act 4, Scene 15"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 4 15 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
if matches_filter 5 2; then
    ...
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 2 --merge "$MERGE" --status 2>/dev/null; then
            ...
            if python "$ANALYZER" "$PLAY" 5 2 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
...
echo "Play: Antony and Cleopatra"
echo "Total scenes: $TOTAL_SCENES"
