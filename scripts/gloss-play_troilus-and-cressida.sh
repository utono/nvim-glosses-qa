...
#
# Usage: ./gloss-play_troilus-and-cressida.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
# Act 5
echo "--- Act 5, Scene 10 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 5 10 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 5, Scene 10 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 5 10 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
