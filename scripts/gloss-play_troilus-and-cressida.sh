#!/bin/bash
# gloss-play_troilus-and-cressida.sh - Analyze the entirety of Troilus and Cressida
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_troilus-and-cressida.sh [OPTIONS] [FILTER]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== Troilus and Cressida - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0
    ...
    # Act 5 Scene 9
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 9 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 9: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 9: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
    # Act 5 Scene 10
    OUTPUT=$(python "$ANALYZER" "$PLAY" 5 10 --merge "$MERGE" --status 2>/dev/null)
    EXIT_CODE=$?
    if [ $EXIT_CODE -eq 0 ]; then
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 10: $CHUNKS chunks (cached)"
        ((CACHED++))
    else
        CHUNKS=$(echo "$OUTPUT" | grep -oP '\d+ chunks' | head -1 | grep -oP '\d+' || echo "?")
        echo "Act 5 Scene 10: $CHUNKS chunks (0 cached, $CHUNKS to process)"
        ((PENDING++))
    fi
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== Troilus and Cressida - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
    ...
    if python "$ANALYZER" "$PLAY" 5 9 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 9"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 9 - NOT FOUND"
    fi
    if python "$ANALYZER" "$PLAY" 5 10 --validate 2>/dev/null; then
        ((VALID++))
        echo "  ✓ Act 5 Scene 10"
    else
        ((INVALID++))
        echo "  ✗ Act 5 Scene 10 - NOT FOUND"
    fi
...
# Act 5, Scene 9
if matches_filter 5 9; then
    echo "--- Act 5, Scene 9 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 9 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 9 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 9: exit code $?"
                echo "[FAILED] Act 5, Scene 9"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 9 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
# Act 5, Scene 10
if matches_filter 5 10; then
    echo "--- Act 5, Scene 10 ---"
    if [ -n "$RESUME_MODE" ]; then
        if python "$ANALYZER" "$PLAY" 5 10 --merge "$MERGE" --status 2>/dev/null; then
            echo "[SKIPPED] Act 5, Scene 10 - fully cached"
            ((SKIPPED++))
        else
            if python "$ANALYZER" "$PLAY" 5 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
                ((SUCCESSFUL++))
            else
                ((ERRORS++))
                FAILED_SCENES="$FAILED_SCENES
  Act 5 Scene 10: exit code $?"
                echo "[FAILED] Act 5, Scene 10"
            fi
        fi
    else
        if python "$ANALYZER" "$PLAY" 5 10 --merge "$MERGE" $DRY_RUN 2>&1 | tee -a "$LOG_FILE"; then
...
echo "Play: Troilus and Cressida"
echo "Total scenes: $TOTAL_SCENES"
