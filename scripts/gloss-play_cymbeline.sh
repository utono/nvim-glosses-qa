...
#
# Usage: ./gloss-play_cymbeline.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
echo ""
echo "--- Act 2, Scene 5 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 2 5 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 2, Scene 5 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 2 5 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
...
echo ""
echo "--- Act 3, Scene 8 ---"
if [ -n "$RESUME_MODE" ] && python "$ANALYZER" "$PLAY" 3 8 --merge "$MERGE" --status 2>/dev/null; then
    echo "[SKIPPED] Act 3, Scene 8 - fully cached"
    ((SKIPPED++))
elif python "$ANALYZER" "$PLAY" 3 8 --merge "$MERGE" $DRY_RUN; then
    ((SUCCESSFUL++))
else
    ((ERRORS++))
    FAILED_SCENES="$FAILED_SCENES
