#!/bin/bash
# gloss-play_winters-tale.sh - Analyze the entirety of The Winter's Tale
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_winters-tale.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== The Winter's Tale - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== The Winter's Tale - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
...
echo "Analyzing The Winter's Tale..."
echo "Play file: $PLAY"
