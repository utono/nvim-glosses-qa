#!/bin/bash
# gloss-play_midsummer-nights-dream.sh - Analyze the entirety of A Midsummer Night's Dream
# Generated by /analyze-plays command
#
# Usage: ./gloss-play_midsummer-nights-dream.sh [--status] [--dry-run] [--resume] [--validate]
#
# Options:
...
if [ -n "$STATUS_ONLY" ]; then
    echo "=== A Midsummer Night's Dream - Cache Status ==="
    echo ""
    CACHED=0
    PENDING=0
...
if [ -n "$VALIDATE_MODE" ]; then
    echo "=== A Midsummer Night's Dream - Validating Script ==="
    echo ""
    VALID=0
    INVALID=0
...
echo "Analyzing A Midsummer Night's Dream..."
echo "Play file: $PLAY"
...
echo "Play: A Midsummer Night's Dream"
echo "Total scenes: $TOTAL_SCENES"
